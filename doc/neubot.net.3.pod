...
stream.I<recv>(maxlen, recv_success, recv_error=None, compat=False)

stream.I<send>(octets, send_success, send_error=None, compat=False)
...
receives a reference to the stream as its only parameter.  The I<compat>
parameter specifies whether the module machinery is allowed to attempt
a non-blocking recv immediately or it should stay compatible with the
B<neubot.http> module code that do not expect this behavior.
...
receives a reference to the stream as its only parameter.  The I<compat>
parameter specifies whether the module machinery is allowed to attempt
a non-blocking send immediately or it should stay compatible with the
B<neubot.http> module code that do not expect this behavior.
