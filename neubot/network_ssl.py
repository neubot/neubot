...
class SSLSocket(object):
    ...
    def sock_recv(self, param):
        """ Start an asynchronous recv() operation """
        self.sock_recv_param = param
        if not self.ssl_hijack_recv:
            self.sock_poller.set_read(self)
        self.ssl_recv_pending = 1
    ...
    def _ssl_recv(self):
        ...
        if self.ssl_hijack_recv:
            self.ssl_hijack_recv = 0
            self.sock_poller.set_write(self)
            if not self.ssl_recv_pending:
                self.sock_poller.unset_read(self)
            self._ssl_send()
            return
        ...
        self.ssl_recv_pending = 0
        try:
            result = self.sock_socket.read(self.sock_recv_param)
        except ssl.SSLError:
            error = sys.exc_info()[1]
            if error.args[0] == ssl.SSL_ERROR_WANT_READ:
                ...
                self.sock_poller.set_write(self)
                self.ssl_recv_pending = 1
                self.ssl_hijack_send = 1
            else:
                logging.warning("network: SSL_read() failed: %s", error)
                self.sock_recv_param = 0
                self.sock_recv_complete(error, None)
        else:
            self.sock_recv_param = 0
            self.sock_recv_complete(None, result)

    def sock_recv_complete(self, error, result):
        """ Invoked when recv is complete """

    def sock_send(self, param):
        """ Start an asynchronous send() operation """
        self.sock_send_param = param
        if not self.ssl_hijack_send:
            self.sock_poller.set_write(self)
        self.ssl_send_pending = 1
    ...
    def _ssl_send(self):
        ...
        if self.ssl_hijack_send:
            self.ssl_hijack_send = 0
            self.sock_poller.set_read(self)
            if not self.ssl_send_pending:
                self.sock_poller.unset_write(self)
            self._ssl_recv()
            return
        ...
        self.ssl_send_pending = 0
        try:
            result = self.sock_socket.write(self.sock_send_param)
        except ssl.SSLError:
            error = sys.exc_info()[1]
            if error.args[0] == ssl.SSL_ERROR_WANT_WRITE:
                self.sock_poller.set_write(self)
                self.ssl_send_pending = 1
            elif error.args[0] == ssl.SSL_ERROR_WANT_READ:
                self.sock_poller.set_read(self)
                self.ssl_send_pending = 1
                self.ssl_hijack_recv = 1
            else:
                logging.warning("network: SSL_write() failed: %s", error)
                self.sock_send_param = None
                self.sock_send_complete(error, 0)
        else:
            self.sock_send_param = None
            self.sock_send_complete(None, result)

    def sock_send_complete(self, error, result):
    ...
    def ssl_handshake(self, server_side, certfile):
        ...
        try:
            ...
            error = sys.exc_info()[1]
            logging.warning("network: ssl.SSLSocket failed: %s", error)
    ...
    def _ssl_do_handshake(self):
        ...
        try:
            ...
            if error.args[0] == ssl.SSL_ERROR_WANT_READ:
                ...
                logging.warning("network: SSL_handshake failed: %s", error)
                self.ssl_handshake_complete(error)
        else:
...
class BuffSSLSocket(SSLSocket):
    ...
    def __init__(self, poller):
        SSLSocket.__init__(self, poller)
