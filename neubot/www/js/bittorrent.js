...
var bittorrent = (function() {
    ...
    self.get_results = function(callbacks, since, unit, until) {
        ...
        var url = "/api/bittorrent?since=" + since_s;
        if (until != undefined) {
    ...
    self.results_formatter_table = function(data, since, unit, until) {
        ...
        html += '<center><table id="bittorrent">';
        html += "<thead><tr>";
    ...
    self.results_formatter_plot = function(data, since, unit, until) {
        ...
        if (mydata.length) {
            ...
            var plot = jQuery.jqplot("chartdiv1", mydata, {
              title: {
                text: i18n.get("Your bittorrent download and upload speed"),
                fontSize: "16pt"
              },
              axes: {
                xaxis: xaxis,
...
function bittorrent_init() {
    ...
    bittorrent.get_recent_results([bittorrent.results_formatter_table,
                                  bittorrent.results_formatter_plot]);
    ...
    var confirm = function() {
        ...
        bittorrent.get_results([bittorrent.results_formatter_table,
                               bittorrent.results_formatter_plot], since);
        return false;
    };
