# neubot/dashtest/http_client.py
...
#
# Autogenerated
#   from m4/neubot/http_client.m4
#   using ./scripts/m4_to_py
#   on Tue Sep 17 23:42:50 CEST 2013
#   by simone
#
...
from neubot.dashtest.network_core import BuffStreamSocket
...
class HTTPClient(BuffStreamSocket):
    """ HTTP client """
    ...
    def __init__(self, poller):
        ...
        self.http_response_trailers = {}

    def handle_close(self):
        BuffStreamSocket.handle_close(self)
        logging.debug("http: handle_close(): %s", self)
        #
    ...
    def _http_read_headers_loop(self):
        """ Loop that reads response headers """
        ...
        while True:
            ...
            if not line:
                break
            self.http_response_lines.append(line)
            if len(self.http_response_lines) > HTTP_MAX_HEADERS:
        ...
        self.http_response_protocol = vector[0]
        self.http_response_code = vector[1]
        self.http_response_reason = vector[2]

        if not self.http_response_protocol.startswith(six.b("HTTP/")):
            logging.warning("http: invalid protocol")
            self.http_handle_headers(HTTP_EINVALIDPROTOCOL)
            return

        if self.http_response_protocol[5:] not in (six.b("1.1"), six.b("1.0")):
            logging.warning("http: unsupported protocol version")
            self.http_handle_headers(HTTP_EUNSUPPORTEDVERSION)
            return

        #
        # Grok response headers (or trailers)
        #

        last_header = None
        for line in self.http_response_lines[1:]:
            ...
            if last_header and line[0:1] in (six.b(" "), six.b("\t")):
                value  = self.http_response_headers[last_header]
                value += six.b(" ")
                value += line.strip()
                # Make sure there are no leading or trailing spaces
                self.http_response_headers[last_header] = value.strip()
                continue
            ...
            if index >= 0:
                ...
                value = value.strip()
                if name not in self.http_response_headers:
                    ...
                    #
                    self.http_response_headers[name] += six.b(", ")
                    self.http_response_headers[name] += value
                last_header = name
                continue
    ...
    def _http_read_headers_hook(self, error, data):
        """ Socket-level hook for reading response headers """
        if error:
    ...
    def http_read_body(self):
        ...
        tmp = self.http_response_headers.get(six.b("transfer-encoding"))
        if tmp == six.b("chunked"):
            logging.debug("http: there is a chunked message body")
            self._http_read_chunked()
            return

        tmp = self.http_response_headers.get(six.b("content-length"))
        if tmp:
            try:
                length = int(tmp)
            except ValueError:
    ...
    def _http_read_bounded_loop(self):
        """ Loop that reads the bounded response body """
        while True:
            ...
            for data in vector:
                ...
                self.http_response_body.append(data)
    ...
    def _http_read_bounded_hook(self, error, data):
        """ Socket-level hook for reading the bounded response body """
        if error:
            logging.warning("http: socket-level error")
    ...
    def _http_read_unbounded(self):
        """ Read unbounded response body """
        self._http_read_unbounded_loop()

    def _http_read_unbounded_loop(self):
        """ Loop that reads unbounded response body """
        while True:
            amount = HTTP_PIECE_MAX
            vector = self.buff_ibuff_readn(amount, False)
            if not vector:
                self.sock_recv_complete = self._http_read_unbounded_hook
                self.sock_recv(HTTP_RECV_MAX)
                return
            for data in vector:
                self.http_response_body.append(data)
    ...
    def _http_read_unbounded_hook(self, error, data):
        """ Socket-level hook for reading unbounded response body """
        if error:
            logging.warning("http: socket-level error")
            self.http_handle_body(HTTP_ESOCKET)
            return
    ...
    def _http_read_chunked_loop(self):
        """ Loop that reads the chunked response body """
        while True:
            ...
            if self.http_chunked_state == 1:
                ...
                for line in self.http_response_lines[1:]:
                    ...
                    #
                    if last_header and line[0:1] in (six.b(" "), six.b("\t")):
                        value  = self.http_response_trailers[last_header]
                        value += six.b(" ")
                        value += line.strip()
                        # Make sure there are no leading or trailing spaces
                        self.http_response_trailers[last_header] = value.strip()
                        continue
                    ...
                    if index >= 0:
                        ...
                        name = name.strip().lower()
                        value = value.strip()
                        if name not in self.http_response_trailers:
                            self.http_response_trailers[name] = value
                        else:
                            #
                            # For headers whose value is a list of
                            # comma-separated values, multiple headers
                            ...
                            #     (See: RFC2616, sect. 4.2)
                            #
                            self.http_response_trailers[name] += six.b(", ")
                            self.http_response_trailers[name] += value
                        last_header = name
                        continue
    ...
    def _http_read_chunked_hook(self, error, data):
        """ Socket-level hook for reading chunked response body """
        if error:
            logging.warning("http: socket-level error")
            self.http_handle_body(HTTP_ESOCKET)
            return
    ...
    def http_append_request(self, method, uri, protocol, headers, body):
        """ Append request to output buffer """
        vector = []

        self.http_request_method = six.b(method)
        self.http_request_uri = six.b(uri)
        self.http_request_protocol = six.b(protocol)

        request_line = six.b(" ").join([self.http_request_method,
          self.http_request_uri, self.http_request_protocol])
        logging.debug("> %s", request_line)
        vector.append(request_line)
        vector.append(six.b("\r\n"))
